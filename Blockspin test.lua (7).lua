...
local MiscTab = Window:Tab({Title = "Misc", Icon = ""})
...
local ServerTab = Window:Tab({Title = "Server", Icon = ""})



--// Services (จำเป็นจริงทั้งหมด)
local Players = game:GetService("Players")
...
task.spawn(function()
    while Running do
        ...
        if ServerCodeLabel and ServerCodeLabel.Set then
            ServerCodeLabel:Set(text)
        elseif ServerCodeLabel then
            ServerCodeLabel.Code = text
        end
...
ServerTab:Button({
    ...
    Callback = function()
        ...
        local req = game:HttpGet(string.format(
